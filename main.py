...
def spawn_fruit():
    ...
    if side == 0: # LEFT
        ...
        vx = random.randint(15, 25)   #  MUCH FASTER
        ...
        vy = random.randint(10, 18)   # <FASTER DROP
    else: # BOTTOM
        x = random.randint(100, WIDTH - 100)
        y = HEIGHT
        vx = random.randint(-6, 6)
        vy = -random.randint(20, 28)  # <HIGHER TOSS
        
    return {'x': x, 'y': y, 'vx': vx, 'vy': vy, 'color': random.choice(FRUIT_COLORS)}

def spawn_explosion(x, y, color):
    for _ in range(15): # More particles for big fruits
        particles.append({
            'x': x, 'y': y,
...
while True:
    ...
    # 3. Spawn Fruits (Increased to 5% chance for chaos)
    ...
    for fruit in fruits[:]: 
        ...
        if (fruit['y'] > HEIGHT + 100 or fruit['y'] < -100 or 
            fruit['x'] < -100 or fruit['x'] > WIDTH + 100):
            fruits.remove(fruit)
        else:
            if not sliced:
                #BIGGER SIZE FRUIT
                cv2.circle(img, (int(fruit['x']), int(fruit['y'])), 70, fruit['color'], cv2.FILLED)
                cv2.circle(img, (int(fruit['x'])-20, int(fruit['y'])-20), 15, (255, 255, 255), cv2.FILLED)
    ...
    for p in particles[:]:
        ...
        if p['life'] <= 0:
            ...
            cv2.circle(img, (int(p['x']), int(p['y'])), 8, p['color'], cv2.FILLED)
